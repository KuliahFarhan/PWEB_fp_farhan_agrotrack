...
if (request_method() === 'POST') {
    try {
        ...
        if ($nama === '' || $luas <= 0) {
            throw new RuntimeException('Nama lahan wajib diisi dan luas manual harus lebih dari 0.');
        }
        ...
        if ($id > 0) {
            $stmt = db()->prepare('UPDATE lahan SET nama_lahan=?, kode_lahan=?, luas=?, tanaman_id=?, komoditas=?, lokasi=?, status=?, catatan=? WHERE id=? AND user_id=?');
            $stmt->execute([$nama, $kode, $luas, $tanamanId, $crop['nama'], $lokasi, $status, $catatan, $id, $userId]);
            ...
            $stmt->execute([$userId, $nama, $kode, $luas, $tanamanId, $crop['nama'], $lokasi, $status, $catatan]);
...
<html lang="id">
  ...
  <body data-portal="petani" data-active="lahan" data-base="../../">
    ...
    <main class="app-main">
      <div class="topbar"><div><h1 class="page-title">Data Lahan</h1><p class="page-kicker">Lahan yang tampil hanya milik akun petani yang sedang login.</p></div></div>
        ...
        <div class="col-xl-4">
          <div class="panel h-100">
            ...
            <form method="post" class="vstack gap-3">
              ...
              <div><label class="form-label fw-semibold">Luas Manual (Ha)</label><input class="form-control" name="luas" type="number" step="0.01" min="0.01" value="<?= e($editing['luas'] ?? '1.00') ?>" required /></div>
          ...
          <div class="panel">
            ...
            <div class="table-responsive">
              <table class="table align-middle">
                <thead><tr><th>Lahan</th><th>Tanaman</th><th>Luas</th><th>Polygon</th><th>Status</th><th>Aksi</th></tr></thead>
                <tbody>
                  ...
                  <?php endforeach; ?>
                  <?php if (!$rows): ?><tr><td colspan="6" class="text-center text-secondary py-4">Belum ada lahan. Tambahkan lahan pertama untuk mulai memakai AgroTrack.</td></tr><?php endif; ?>
                </tbody>
              </table>
            </div>
          </div>
